  ...
  const fetchUnits = async () => {
    try {
      const res = await axios.get(
        "http://localhost:9080/units/getYourUnits?role=INSTRUCTOR",
        ...
        <tbody>
          {units?.map((unit) => (
            ...
            >
              ...
              <td>
                {" "}
                {unit.assignments.reduce((total, each) => {
                  return total + (each.submissionsMade || 0);
                }, 0)}
              </td>
              {unit.id === hoverId ? (
                <div className="AssignmentSubmitBtnDiv">
                  <button onClick={() => HandleCreateAssingment(unit.id)}>
                    Add Assignment
                  </button>
                </div>
              ) : null}
